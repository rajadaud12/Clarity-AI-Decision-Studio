...
function ollamaConnectionError() {
  return new Error("Ollama Cloud could not be reached after three attempts. Please retry in a moment, or select OpenAI Luna in Settings.");
}
...
async function callOllama(args: {
  ...
  for (let attempt = 0; attempt < MAX_OLLAMA_ATTEMPTS; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        if (isRetryableOllamaStatus(response.status) && attempt < MAX_OLLAMA_ATTEMPTS - 1) {
      ...
      const isConnectionFailure = error instanceof TypeError || (error instanceof Error && error.name === "TimeoutError");
      if (isConnectionFailure) {
        if (attempt < MAX_OLLAMA_ATTEMPTS - 1) {
          await pause(500 * 2 ** attempt);
          continue;
        }
